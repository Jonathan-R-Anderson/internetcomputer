...
GREMLIN_TERM_EXE_NAME = gremlin_terminal
GREMLIN_TERM_EXE = $(BUILD_DIR)/$(GREMLIN_TERM_EXE_NAME)
...
$(ISO_FILE): $(KERNEL_BIN) $(GREMLIN_SHELL_EXE) $(GREMLIN_TERM_EXE)
	...
	@if [ -f "$(GREMLIN_TERM_EXE)" ]; then \
	echo "Copying '$(GREMLIN_TERM_EXE)' to '$(strip $(ISO_BIN_DIR))/$(GREMLIN_TERM_EXE_NAME)'"; \
	cp $(GREMLIN_TERM_EXE) $(ISO_BIN_DIR)/$(GREMLIN_TERM_EXE_NAME); \
	else \
	echo "Warning: Gremlin Terminal executable not found at $(GREMLIN_TERM_EXE). ISO will not include it."; \
	fi
